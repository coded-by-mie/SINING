...
if(isset($_POST['selectedOption'])) {
  ...
  if(isset($_GET['remove'])){
     ...
     mysqli_query($conn, "UPDATE `sining_artworks` SET `purchased`='1' WHERE artId = '$remove_id'");
     mysqli_query($conn, "UPDATE `sining_artworks1` SET `purchased`='1' WHERE artId = '$remove_id'");
...
<div class="container cont">
  ...
  <button class="tablinks" onclick="openCity(event, 'tobeApproved')" id="defaultOpen">To be Approved</button>
	<button class="tablinks" onclick="openCity(event, 'bank')">Pay Through Bank</button>
  <button class="tablinks" onclick="openCity(event, 'gcash')">Pay Through Gcash</button>
...
<div id="tobeApproved" class="tabcontent">
  ...
  <?php
 		...
 		echo '<h2>₱'.number_format($fetch_artist['product_price'], 2).'</h2>';
...
<div id="gcash" class="tabcontent">
<h1>To Pay</h1>
<?php
 		$user_info = mysqli_query($conn, "SELECT * FROM `product_status` WHERE buyer_id = '$user_id' AND product_status = 'To pay' AND payment_method = 'Gcash'");
 		if(mysqli_num_rows($user_info) > 0){
            while($fetch_artist = mysqli_fetch_assoc($user_info)){	
 		echo '<img src="seller_file/artworks/seller_'.$fetch_artist['seller_id'].'/'.$fetch_artist['product_image'].'" alt="" width=300 class="img-fluid">';
 		echo '<h3>'.$fetch_artist['product_name'].'</h3>';
    ...
    echo '<button type="button" class="dlt" onclick="getOrderID('.$fetch_artist['id'].')">PAY</button>';
...
function getOrderID(order_id){
    ...
    $.ajax({
            ...
            success: function(result){
            window.location.href = "upload_gcash.php";
    }
  });	
}
</script>
